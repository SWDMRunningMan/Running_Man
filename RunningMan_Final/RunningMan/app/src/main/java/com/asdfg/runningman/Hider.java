...
public class Hider extends AppCompatActivity implements SensorEventListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        userName = intent.getStringExtra("userName"); // 로그인 화면 아이디
        ...
        for(int i=0;i<(num_H+1)/2;i++) { // i<(Integer.valueOf(hiderNum)+1)/2
            ...
            for (int j = 0; j < (num_H+1)%2+1; j++) {
                ...
                if(hider2.get(i*2+j).equals(userName)) // if(hider2.get(i*2+j).equals(userName))
                    index = i*2+j;
        ...
        dd.start();
        pictureThread tt = new pictureThread(sock);
        tt.start();
    ...
    private void _readMessage()
    {
        ...
        //    Toast.makeText(getApplicationContext(),msgs.get(0) + " : " + seeker.get(q),Toast.LENGTH_SHORT).show();
            // 아웃처리
            if (msgs.get(0).equals(seeker.get(q)))  // msgs.get(0) == nfc로 전해받은 상대의 맥주소
            {
              //  Toast.makeText(getApplicationContext(),msgs.get(0) + " : " + seeker.get(q),Toast.LENGTH_SHORT).show();
                // 아웃처리
                try {
                    outstream.writeUTF("401 "+rID+" " +ID);
                    outstream.flush();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        try {
           ...
           int lnth = byteArray.length;
            // rotated 서버에 전송
            try {
                outstream.writeUTF("400 "+rID+" " +ID + " " + lnth ); outstream.flush();
                for(int t=0;t<lnth;t++){
                 outstream.writeByte(byteArray[t]);
                outstream.flush();}
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    class pictureThread extends Thread {
        ...
        DataOutputStream outstream;
        DataInputStream instream;
        public pictureThread(Socket sc) {
            socket = sc;
        }
        ...
        public void run(){
            ...
            while (true) { try{
            place = 0;
            int l = instream.readInt();
            if (l == 400) {
                String capturedID = instream.readUTF();
                int size = instream.readInt();
                for (int q = 0; q < num_H; q++) {
                    if (hider.get(q).equals(capturedID))
                        place = q;
                }
                byte[] temp = new byte[size];
                for (int q = 0; q < size; q++) {
                    temp[q] = instream.readByte();
                }
                final Bitmap captured = BitmapFactory.decodeByteArray(temp, 0, size);
                handler.post(new Runnable() {
                    public void run() {
                        tempImage[place].setImageBitmap(captured);
                    }
            });
            }
            if(l==401){
                alive--;
                String uuID = instream.readUTF();
                for(int q=0;q<num_H;q++){
                    if(hider.get(q).equals(uuID)){
                        final int y = q;
                        handler.post(new Runnable(){
                            public void run(){
                                final Drawable myImage = getResources().getDrawable(R.drawable.fail);
                                tempImage[y].setImageDrawable(myImage);
                            }
                        });
                }}
            }
                    ...
                    if(l==501){
                        ...
                        intent2.putExtra("step",String.valueOf(step));
                        intent2.putExtra("user",user);
                        intent2.putExtra("size",String.valueOf(size));
                        intent2.putExtra("steparray",stepArray);
                        // step 서버에 보내기
